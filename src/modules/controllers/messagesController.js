...
export const getDialogues = async (req, res, next) => {
  try {
    ...
    if (dialogues.length) {
      if (req.payload.id !== dialogues[0].to)
...
export const getDialogue = async (req, res, next) => {
  try {
...
export const getMessages = async (req, res, next) => {
  try {
    ...
    if (!dialogueId)
      return next(createError.BadRequest("dialogueId must not be empty"));
  ...
  try {
    upload(req, res, async (err) => {
      ...
      if (isNewDialogue) {
        ...
        req.io
          .to("dialogues:" + to)
          .emit("updateDialogue", newOrUpdatedDialogue);
      }
...
export const deleteMessage = async (req, res, next) => {
  try {
    ...
    if (!dialogueId)
      return next(createError.BadRequest("dialogueId must not be empty"));
    if (!groupId)
      return next(createError.BadRequest("groupId must not be empty"));
    if (!messageId)
      return next(createError.BadRequest("messageId must not be empty"));
    ...
    const messages = await Message.find({
      dialogueId: Types.ObjectId(dialogueId),
    }).sort({ createdAt: -1 });
    if (messages.length) {
      await Dialogue.updateOne(
        { _id: Types.ObjectId(dialogueId) },
        { lastMessage: messages[0]._id, updatedAt: messages[0].createdAt }
      );
    } else {
      const dialogue = await Dialogue.findById(dialogueId);
    ...
    req.io
      .to("pm:" + dialogueId)
      .emit("messageDeleted", { id: messageId, groupId });
  } catch (err) {
    next(createError.InternalServerError(err));
  }
};
