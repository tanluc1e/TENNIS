...
import { checkFileExec, videoTypes } from "../utils/checkFileExec.js";
import storage from "../utils/storage.js";
import createThumb from "../utils/createThumbnail.js";
  ...
  try {
    ...
    if (sort === "popular") {
      boards = await Board.paginate(
        {},
        {
          sort: { threadsCount: -1, answersCount: -1 },
          page,
          limit,
...
export const getBoard = async (req, res, next) => {
  try {
    ...
    if (name) {
      ...
      return next(
        createError.BadRequest("Board name or boardId must not be empty")
      );
    }
...
export const createBoard = async (req, res, next) => {
  try {
    ...
    if (name.trim() === "")
      return next(createError.BadRequest("Board name must not be empty"));
    if (title.trim() === "")
      return next(createError.BadRequest("Board title must not be empty"));
    ...
    if (nameExist)
      return next(
        createError.Conflict(
          "Board with this short name is already been created"
        )
      );
...
export const getRecentlyThreads = async (req, res, next) => {
  try {
    ...
    const threads = await Thread.paginate(
      {},
      {
        sort: { pined: -1, newestAnswer: -1, createdAt: -1 },
        page,
        limit,
        populate,
      }
  ...
  try {
    ...
    if (sort === "answersCount") {
      threads = await Thread.paginate(
        { boardId },
        { sort: { pined: -1, answersCount: -1 }, page, limit, populate }
      );
    } else if (sort === "newestAnswer") {
      threads = await Thread.paginate(
        { boardId },
        { sort: { pined: -1, newestAnswer: -1 }, page, limit, populate }
      );
    } else {
      threads = await Thread.paginate(
        { boardId },
        { sort: { pined: -1, createdAt: -1 }, page, limit, populate }
      );
    }
...
export const createThread = async (req, res, next) => {
  try {
    upload(req, res, async (err) => {
        ...
        await Promise.all(
          req.files.map(async (item) => {
            if (videoTypes.find((i) => i === item.mimetype)) {
              const thumbFilename = item.filename.replace(
                path.extname(item.filename),
                ".jpg"
              );

              const thumbnail = await createThumb(
                item.path,
                "forum",
                thumbFilename
              );
      ...
      await Board.updateOne(
        { _id: Types.ObjectId(boardId) },
        { $inc: { threadsCount: 1 }, newestThread: now }
      );
...
export const deleteThread = async (req, res, next) => {
  try {
    ...
    await Board.updateOne(
      { _id: Types.ObjectId(thread.boardId) },
      {
        $inc: {
          threadsCount: -1,
...
export const clearThread = async (req, res, next) => {
  try {
    const { threadId } = req.body;
    const moder = req.payload.role >= 2;
    ...
    await Thread.updateOne(
      { _id: Types.ObjectId(threadId) },
      { answersCount: 0 }
    );
    await Board.updateOne(
      { _id: Types.ObjectId(thread.boardId) },
      { $inc: { answersCount: -answersCount } }
    );
...
export const adminEditThread = async (req, res, next) => {
  try {
    upload(req, res, async (err) => {
      if (err) return next(createError.BadRequest(err.message));

      const { threadId, title, body, pined, closed } = JSON.parse(
        req.body.postData
      );
      const moder = req.payload.role >= 2;
  ...
  try {
    upload(req, res, async (err) => {
      ...
      if (!threadId)
        return next(createError.BadRequest("threadId must not be empty"));
      if (body.trim() === "")
        return next(createError.BadRequest("Answer body must not be empty"));
      ...
      await Board.updateOne(
        { _id: Types.ObjectId(thread.boardId) },
        { $inc: { answersCount: 1 }, newestAnswer: now }
      );
      await Thread.updateOne(
        { _id: Types.ObjectId(threadId) },
        { $inc: { answersCount: 1 }, newestAnswer: now }
...
export const deleteAnswer = async (req, res, next) => {
  try {
    ...
    if (!moder) return next(createError.Unauthorized("Action not allowed"));
    if (!answerId)
      return next(createError.BadRequest("answerId must not be empty"));
    ...
    if (req.payload.role < answer.author.role)
      return next(createError.Unauthorized("Action not allowed"));
    ...
    await Board.updateOne(
      { _id: Types.ObjectId(answer.boardId) },
      { $inc: { answersCount: -1 } }
    );
    await Thread.updateOne(
      { _id: Types.ObjectId(answer.threadId) },
      { $inc: { answersCount: -1 } }
    );
    ...
    req.io
      .to("thread:" + answer.threadId)
      .emit("answerDeleted", { id: answerId });
  } catch (err) {
    next(createError.InternalServerError(err));
  }
};
...
export const editAnswer = async (req, res, next) => {
  try {
    upload(req, res, async (err) => {
      ...
      if (
        req.payload.id === answer.author._id ||
        req.payload.role < answer.author.role
      ) {
        return next(createError.Unauthorized("Action not allowed"));
      }
